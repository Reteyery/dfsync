...
public class HttpsUtils {
	...
	private static SSLParams getSslSocketFactoryBase(X509TrustManager trustManager, InputStream bksFile, String password, InputStream... certificates) {
		...
		try {
			KeyManager[] e = prepareKeyManager(bksFile, password);
			...
			sslContext.init(e, new TrustManager[]{manager}, (SecureRandom)null);
	...
	private static KeyManager[] prepareKeyManager(InputStream bksFile, String password) {
		try {
			if(bksFile != null && password != null) {
				...
				kmf.init(e, password.toCharArray());
	...
	private static TrustManager[] prepareTrustManager(InputStream... certificates) {
		if(certificates != null && certificates.length > 0) {
			try {
				...
				int index = 0;
				InputStream[] tmf = certificates;
				int var5 = certificates.length;

				for(int var6 = 0; var6 < var5; ++var6) {
					InputStream certStream = tmf[var6];
					String certificateAlias = Integer.toString(index++);
					Certificate cert = e.generateCertificate(certStream);
